...
public class ValidateUsername extends Validation {
    ...
    public void validate(Details detail) {
        String name = detail.getUsername().getText().replace("Enter User Name...", "").trim();
        Border border;
        Color color;
        if (name.isEmpty()) {
            color = new Color(184, 15, 10);
            border = BorderFactory.createLineBorder(color, 1);
            detail.getUsername().setBorder(border);
            detail.getUsername().requestFocus();
            ImageIcon icon = new ImageIcon("src/images/name.png");
            JOptionPane.showMessageDialog(detail.getFrame(), "Need a user name to register", "Error", JOptionPane.ERROR_MESSAGE, icon);
