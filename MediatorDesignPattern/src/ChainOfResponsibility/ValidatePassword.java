...
public class ValidatePassword extends Validation {
    ...
    public void validate(Details detail) {
        ...
        if (password.isEmpty()) {
            ...
            detail.getPassword().requestFocus();
            ImageIcon icon = new ImageIcon("src/images/password.png");
            JOptionPane.showMessageDialog(detail.getFrame(), "Password need minimum eight characters, at least one uppercase "
                    + "letter, one lowercase letter, one number and one special character", "Error", JOptionPane.ERROR_MESSAGE, icon);
        } else {
